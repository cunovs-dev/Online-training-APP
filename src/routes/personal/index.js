  ...
  return (
    <div>
      ...
      <div className={styles.content}>
        <TitleBox title="业务偏好"
          sup={<span style={{ color: '#02B7EE' }}
            onClick={() => handleGoto(dispatch, 'perfectInformation')}
          >重新选择</span>}
        />
        <div className={styles.tagBox}>
          {vocationalList.length > 0 && vocationalList.map(item => (<Tag key={item.id}
            inline
            size="xs"
            text={item.title}
            color="green"
        ...
        />
        <div className={styles.tagBox}>
          {sceneList.length > 0 && sceneList.map(item => (<Tag key={item.id}
            inline
            size="xs"
            text={item.title}
            color="cyan"
