  ...
  return (
    <div>
      ...
      <div className={styles.content}>
        <TitleBox title="业务偏好" sup={<span style={{ color: '#02B7EE' }} onClick={() => handleGoto(dispatch, 'perfectInformation')}>重新选择</span>} />
        <div className={styles.tagBox}>
          {vocationalList.map(item => <Tag key={item.id} inline size="xs" text={item.title} color="green" />)}
        </div>
        <TitleBox title="场景偏好" sup={<span style={{ color: '#02B7EE' }}
                                          onClick={() => handleGoto(dispatch, 'perfectInformation')}>重新选择</span>} />
        <div className={styles.tagBox}>
          {sceneList.map(item => <Tag key={item.id} inline size="xs" text={item.title} color="cyan" />)}
        </div>
        <TitleBox title="短板" sup='' />
        <div className={styles.tagBox}>
          {weaknessList.map(item => <Tag key={item.id} inline size="xs" text={item.title} color="red" />)}
        </div>
      </div>
    </div>
  );
